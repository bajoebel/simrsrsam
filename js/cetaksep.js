        ...
        function cetaksep(nosep,tgl,reg_unit="") {
            var noreg=$('#sepiddaftar').html();
            // alert(noreg)
            var polirs=$('#tujuan').html();
            var noantrian= $('#noantrian').html();
            ...
            var url = url_call_back+"/registrasi/datacetaksep/"+nosep+"/"+tgl+"?regunit="+reg_unit;
            $.ajax({
                url     : url,
                type    : "GET",
                dataType: "json",
                ...
                success : function(data){
                    if(data.status==true){
                        ...
                        doc.addImage(imgData, 'PNG', 15, 6, 45, 10);
                        ...
                        if(data.local==1){
                            ...
                            nmpst = data.seponline.peserta.nama;
                            tgllahir = data.seponline.peserta.tglLahir;
                            // alert(data.seponline.peserta.kelamin);
                            if(data.seponline.peserta.kelamin=='L') jnskelamin='Laki-Laki';
                            else jnskelamin='Perempuan';
                        ...
                        doc.text(58, 15, 'RSUD DR ACHMAD MOCHTAR BUKITTINGGI');
                        doc.setFontSize(12);
                        if(noantrian>0) doc.text(167, 10, 'No Urut '+ noantrian);
                        doc.text(145, 15, potensiprb == 'Potensi PRB' ? 'PASIEN POTENSI PRB' : '');
                        // doc.text(145, 15, 'PASIEN POTENSI PRB');

                        doc.setFontSize(9);
                        doc.text(17, 25, 'No Reg');
                        doc.text(17, 30, 'No.SEP');
                        doc.text(17, 35, 'Tgl.SEP');
                        doc.text(17, 40, 'No.Kartu');
                        doc.text(17, 45, 'Nama Peserta');
                        doc.text(17, 50, 'Tgl.Lahir');
                        doc.text(17, 55, 'No.Telepon');
                        doc.text(17, 60, 'Poli RS/Sub/Spesialis');
                        doc.text(17, 65, 'Dokter');
                        doc.text(17, 70, 'Faskes Perujuk');
                        doc.text(17, 75, 'Diagnosa Awal');
                        doc.text(17, 80, 'Catatan');

                        doc.text(50, 25, ': ' + noreg);
                        doc.text(50, 30, ': ' + nosep);
                        doc.text(50, 35, ': ' + tglsep);
                        doc.text(50, 40, ': ' + nokartu + ' (MR. '+noMr+')');
                        doc.text(50, 45, ': ' + nmpst);
                        doc.text(50, 50, ': ' + tgllahir + ' Kelamin : '+ jnskelamin);
                        doc.text(50, 55, ': ' + notelp);
                        // doc.text(40, 55, ': ' + poli + eksekutif);
                        doc.text(50, 60, ': ' + polirs+" / "+poli);
                        doc.text(50, 65, ': ' + dokter);
                        doc.text(50, 70, ': ' + faskesperujuk);
                        doc.text(50, 75, ': ' + dxawal);
                        doc.text(50, 80, ': ' + catatan);
                        doc.setFontSize(8);
                        doc.text(125, 25, katarak == '1' ? '* PASIEN OPERASI KATARAK' : '');
                        doc.setFontSize(9);
                        // Blok Sebelah Kiri Kiri
                        
                        
                        // doc.text(125, 25, 'No Antri ');
                        doc.text(125, 30, 'Peserta ');
                        doc.text(125, 35, 'COB ');
                        doc.text(125, 40, 'Jns.Rawat ');

                        doc.text(125, 45, 'Jns.Kunjungan ');
                        doc.text(125, 55, 'Poli Perujuk ');
                        doc.text(125, 60, 'Kls.Hak ');
                        doc.text(125, 65, 'Kls.Rawat ');
                        doc.text(125, 80, 'Penjamin ');
                        // doc.text(145, 15, prolanis);
                        // if(noantrian>0) doc.text(150, 30, ': '+noantrian);
                        doc.text(150, 30, ': ' + jnspst);
                        doc.text(150, 35, ': ' + cob.substring(0, 30));
                        doc.text(150, 40, ': ' + jnsrawat);
                        ...
                        doc.text(150, 45, ': - ' + kunjunganText);
                        // alert(data.response.tujuanKunj);
                        // alert(berkelanjutan)
                        if (berkelanjutan != null || berkelanjutan!='') {
                            // alert(berkelanjutan)
                            if (berkelanjutan == '0')
                                doc.text(150, 50, ': - ' + "Prosedur tidak berkelanjutan");
                            else if (berkelanjutan == '1')
                                doc.text(150, 50, ': - ' + "Prosedur dan terapi berkelanjutan");
                            else{
                                if(cetakan>1){
                                    doc.text(150, 50, ': - ' + "Prosedur tidak berkelanjutan");
                                }
                        ...
                        doc.text(150, 55, ': ' + poliPerujuk);
                        doc.text(150, 60, ': ' + hakKelas);
                        doc.text(150, 65, ': ' + klsrawat);
                        ...
                        if (penjamin != null) {
                            if (penjamin != '-') {
                                ...
                                for (var i = 0; i < _penjamin.length; i++) {
                                    ...
                                    var nama=_penjamin[i];
                                    // alert(nama);
                                    if (i == 0) {
                                        doc.text(150, col, ': ' + nama);
                                        _infoJKK = nama;
                                    } else {
                                        doc.text(150, col, '  ' + nama);
