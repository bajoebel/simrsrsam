  ...
  $("#tgl_masuk, #tgl_keluar").on("change textInput input", function () {
    if ($("#tgl_masuk").val() != "" && $("#tgl_keluar").val() != "") {
      ...
      var diffDays = Math.round(
        Math.round((secondDate.getTime() - firstDate.getTime()) / oneDay)
      );
      if (diffDays < 0) {
        $("#los").val("");
        $("#tgl_keluar").val("");
    ...
    success: function (data) {
      ...
      if (data["status"] == true) {
        ...
        for (var i = 0; i < jmlData; i++) {
          ...
          tabel +=
            "<td class='text-right'>Rp. " + res[i]["tarif_layanan"] + "</td>";
          tabel += "<td>" + res[i]["kategori_tarif"] + "</td>";
          tabel += "<td>" + res[i]["kelas_layanan"] + "</td>";
          tabel += "<td class='text-right'>";
          tabel +=
            "<button type='button' class='btn btn-danger ' onclick='pilihTarif(\"" +
            res[i]["idx"] +
            '", "' +
            res[i]["layanan"] +
            '","' +
            ...
            '","' +
            res[i]["kelas_id"] +
            "\")'><span class='fa fa-check' ></span> Pilih</button>";
          tabel += "</td>";
          tabel += "</tr>";
        }
        $("#getTarif").html(tabel);
        ...
        if (data["row_count"] <= limit) {
          ...
          var btnFirst =
            "<button class='btn btn-default btn-sm' onclick='getData(0)'><span class='fa fa-angle-double-left'></span></button>";
          if (curIdx > 1) {
            var prevSt = (curIdx - 1) * data["limit"] - data["limit"];
            btnFirst +=
              "<button class='btn btn-default btn-sm' onclick='getData(" +
              prevSt +
          ...
          $("#pagination").html(
            '<div class="btn-group">' + pagination + "</div>"
          );
        }
      }
      $("#divCariTarif").show();
...
function pilihTarif(a, b, c, d, e, f, g) {
  ...
  if (idKelas == g || g == "0") {
    ...
    //alert(g +"!="+idKelas);
    var x = confirm(
      "Kelas tarif tidak sama dengan Kelas Layanan yang digunakan Pasien !\nApakah anda tetap lanjutkan proses ini?"
    );
    if (x) {
      cekKelas = true;
    }
...
function simpanTindakan() {
  ...
  if (b == "") {
    alert(
      "Ops. No.Reg RS kosong. Pastikan No.Reg RS anda tampil. atau refresh kembali browser anda"
    );
  } else if (c == "") {
    alert(
      "Ops. No.Reg Unit kosong. Pastikan No.Reg Unit anda tampil. atau refresh kembali browser anda"
    );
  } else if (d == "") {
    alert(
      "Ops. No.MR kosong. Pastikan No.MR anda tampil. atau refresh kembali browser anda"
    );
  } else if (e == "") {
    alert(
      "Ops. Kode tarif kosong. Coba ulangi entri tindakan atau refresh kembali browser anda"
    );
  } else if (f == "") {
    alert(
      "Ops. Tindak atau layanan kosong. Coba ulangi entri tindakan atau refresh kembali browser anda"
    );
    ...
    alert(
      "Ops. Group tarif BPJS kosong. Pastikan Group tarif BPJS terisi dengan baik pada master tarif."
    );
  } else if (l == "") {
    alert(
      "Ops. Kelas layanan kosong. Pastikan Kelas layanan terisi dengan baik pada master tarif."
    );
  } else {
    var postData = {};
...
function hapusItem(a, b) {
  ...
  if (pulang <= 0) {
    ...
    alert(
      "Maaf Pasien yang sudah dipulangkan, dan transaksi sudah tidak bisa dihapus"
    );
  }
}
...
function registrasiPasien(a) {
  ...
  var b = $("#ruang_id").val();
  var x = confirm(
    "Apakah anda yakin akan meregistrasikan pasien ini di " + xRuang + "?"
  );
  if (x) {
    if (a == "") {
        ...
        success: function (data) {
          if (data.code == 200) {
            ...
            if (y) {
              ...
              $("#lblruangpengirim").html(
                data.data.nama_ruang_pengirim +
                  "(" +
                  data.data.nama_kamar_pengirim +
                  ")"
              );
              $("#lblruangpenerima").html(data.data.nama_ruang);
              $("#lblnama_dokter_pengirim").html(
                data.data.nama_dokter_pengirim
              );
              getKamar();
...
function batalPulang(a) {
  var x = confirm(
    "Apakah anda yakin akan membatalkan status pulang pasien ini ?"
  );
  if (x) {
    if (a == "") {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status == true) {
        ...
        tampilkanPesan(
          "info",
          "Nomor Antrian " + data.no_antrian_poly + " dipanggil..."
        );
      } else {
