...
class mati_lebih_48 extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $x['content'] = $this->load->view('sirs/ranap/mati_lebih_48/template_form',$y,true);
    ...
    function getReport(){
        ...
        AND jns_layanan='RI' AND id_keadaan_keluar='5') b ON a.idx = b.id_tindakan_pelayanan
        GROUP BY a.idx"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/ranap/mati_lebih_48/getdata',$x);
    }
    function cetak(){
        $x['thn'] = (isset($_GET['thn'])) ? trim($_GET['thn']) : '';
        $SQL = "SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$x[thn]'
        AND jns_layanan='RI' AND id_keadaan_keluar='5'";
        $cek = $this->db->query("$SQL");
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/ranap/mati_lebih_48/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/ranap/mati_lebih_48/getitem',$x);        
    }
}
?>