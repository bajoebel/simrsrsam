...
class toi extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['contentTitle'] = "SIRS - [Turn Over Interval (TOI)]";        
            $y['contentTitle'] .= "<br/>Jns. Layanan : RANAP";   
            $y['index_menu'] = 8;
            $this->db->where('grid','2');     
            $y['datRanap'] = $this->db->get('tbl01_ruang');     
            $x['content'] = $this->load->view('sirs/ranap/toi/template_form',$y,true);
    ...
    function getReport(){
        ...
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='01',los,0)) AS 'M01',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='02',los,0)) AS 'M02',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='03',los,0)) AS 'M03',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='04',los,0)) AS 'M04',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='05',los,0)) AS 'M05',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='06',los,0)) AS 'M06',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='07',los,0)) AS 'M07',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='08',los,0)) AS 'M08',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='09',los,0)) AS 'M09',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='10',los,0)) AS 'M10',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='11',los,0)) AS 'M11',
        SUM(IF(DATE_FORMAT(tgl_keluar,'%m')='12',los,0)) AS 'M12'
        ...
        $x['ranap'] = $ranap;
        $this->load->view('sirs/ranap/toi/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/ranap/toi/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['ranap'] = $ranap;
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/ranap/toi/getitem',$x);        
    }
}
?>