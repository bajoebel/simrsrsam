...
class morbodotas_sl extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['index_menu'] = 8;
            $y['contentTitle'] = "SIRS - [Morboditas Mortalitas (Sebab Luar)]";        
            $y['contentTitle'] .= "<br/>Jns. Layanan : RANAP";        
            $x['content'] = $this->load->view('sirs/ranap/morbodotas_sl/template_form',$y,true);
    ...
    function getReport(){
        ...
        $this->load->view('sirs/ranap/morbodotas_sl/getdata',$x);
    }
    function cetak(){
        $x['thn'] = (isset($_GET['thn'])) ? trim($_GET['thn']) : '';
        $SQL = "SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_keluar,'%Y') = '$x[thn]'
        AND jns_layanan='RI' AND kecelakaan='1'";
        $cek = $this->db->query("$SQL");
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/ranap/morbodotas_sl/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/ranap/morbodotas_sl/getitem',$x);        
    }
}
?>