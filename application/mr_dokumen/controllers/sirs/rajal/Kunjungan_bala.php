...
class kunjungan_bala extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $x['content'] = $this->load->view('sirs/rajal/kunjungan_bala/template_form',$y,true);
    ...
    function getReport(){
        ...
        (SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$thn' AND jns_layanan='RJ') b 
        ON a.jns_kunjungan=b.jns_kunjungan
        GROUP BY a.jns_kunjungan";
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_bala/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/rajal/kunjungan_bala/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_bala/getitem',$x);        
    }
}
?>