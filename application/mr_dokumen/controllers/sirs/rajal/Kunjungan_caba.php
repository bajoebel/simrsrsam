...
class kunjungan_caba extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $x['content'] = $this->load->view('sirs/rajal/kunjungan_caba/template_form',$y,true);
    ...
    function getReport(){
        ...
        AND jns_layanan='RJ') b ON a.idx = b.id_cara_bayar
        GROUP BY a.idx"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_caba/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/rajal/kunjungan_caba/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_caba/getitem',$x);        
    }
}
?>