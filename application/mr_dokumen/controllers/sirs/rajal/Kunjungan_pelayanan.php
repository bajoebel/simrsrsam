...
class kunjungan_pelayanan extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['index_menu'] = 7;
            $y['contentTitle'] = "SIRS - [Kunjungan per Pelayanan]";        
            $y['contentTitle'] .= "<br/>Jns. Layanan : RAJAL";        
            $x['content'] = $this->load->view('sirs/rajal/kunjungan_pelayanan/template_form',$y,true);
    ...
    function getReport(){
        ...
        AND jns_layanan='RJ') b ON a.idx = b.id_tindakan_pelayanan
        GROUP BY a.idx"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_pelayanan/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/rajal/kunjungan_pelayanan/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_pelayanan/getitem',$x);        
    }
}
?>