...
class kunjungan_poli extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['index_menu'] = 7;
            $y['contentTitle'] = "SIRS - [Kunjungan per Poliklinik]";        
            $y['contentTitle'] .= "<br/>Jns. Layanan : RAJAL";        
            $x['content'] = $this->load->view('sirs/rajal/kunjungan_poli/template_form',$y,true);
    ...
    function getReport(){
        ...
        FROM (SELECT * FROM tbl01_ruang WHERE grid=1) a LEFT JOIN 
        (SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$thn'
        AND jns_layanan='RJ') b ON a.idx = b.id_ruang
        GROUP BY a.idx"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_poli/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/rajal/kunjungan_poli/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/rajal/kunjungan_poli/getitem',$x);        
    }
}
?>