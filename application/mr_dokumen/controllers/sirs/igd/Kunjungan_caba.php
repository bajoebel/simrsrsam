...
class kunjungan_caba extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['index_menu'] = 6;
            $y['contentTitle'] = "SIRS - [Kunjungan per Cara Bayar]";        
            $y['contentTitle'] .= "<br/>Jns. Layanan : IGD";        
            $x['content'] = $this->load->view('sirs/igd/kunjungan_caba/template_form',$y,true);
    ...
    function getReport(){
        ...
        FROM tbl01_cara_bayar a LEFT JOIN 
        (SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$thn' 
        AND jns_layanan='GD') b ON a.idx = b.id_cara_bayar
        GROUP BY a.idx"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/igd/kunjungan_caba/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/igd/kunjungan_caba/print',$x);
        }else{
    ...
    function getdata(){
        ...
        SUM(IF(DATE_FORMAT(tgl_masuk,'%m')='12',1,0)) AS '12'
        FROM tbl01_cara_bayar a LEFT JOIN 
        (SELECT * FROM tbl07_report_sirs WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$thn' AND jns_layanan='GD') b ON a.idx = b.id_cara_bayar
        GROUP BY a.idx"; 

        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/igd/kunjungan_caba/getitem',$x);        
    }
}
?>