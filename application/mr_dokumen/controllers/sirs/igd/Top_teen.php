...
class top_teen extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $x['content'] = $this->load->view('sirs/igd/top_teen/template_form',$y,true);
    ...
    function getReport(){
        ...
        WHERE DATE_FORMAT(tgl_masuk,'%Y') = '$thn' AND jns_layanan='GD'
        GROUP BY kode_icd 
        ORDER BY num_kunjungan DESC"; 
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/igd/top_teen/getdata',$x);
    }
    function cetak(){
        ...
        if ($cek->num_rows() > 0) {
            $x['SQL'] = $cek;
            $this->load->view('sirs/igd/top_teen/print',$x);
        }else{
    ...
    function getdata(){
        ...
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('sirs/igd/top_teen/getitem',$x);        
    }
}
?>