...
class penyimpanan_berkas extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['index_menu'] = 2;
            $y['contentTitle'] = "Data Penyimpanan Berkas";        
            $x['content'] = $this->load->view('penyimpanan_berkas/template_table',$y,true);
    ...
    function getView(){
        ...
        $condition = "WHERE a.idx NOT IN (SELECT id_penyimpanan_berkas FROM tbl06_penyimpanan_berkas_batal) ";
        ...
        $SQL = "SELECT a.*,COUNT(b.idx) AS jmlDokumen FROM tbl06_penyimpanan_berkas a 
        LEFT JOIN tbl06_penyimpanan_berkas_item b ON a.idx=b.id_penyimpanan_berkas
        $condition GROUP BY a.idx ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target']      = 'tbody#getdata';
        $config['uri_segment']  = $this->uri_segment;
        $config['base_url']    = base_url().'mr_dokumen.php/penyimpanan_berkas/getView';
        ...
        $this->load->view('penyimpanan_berkas/getdata',$x);
    }
    function tambah(){
        $ses_state = $this->users_model->cek_session_id();
        if($ses_state){
            ...
            $y['contentTitle'] = "Penyimpanan Berkas";        

            $id_penyimpanan_berkas = $this->uri->segment(3);
            if(empty($id_penyimpanan_berkas)){
                $params = array(
                    'userID' => $this->session->userdata('get_uid')
                );
                $cekCommand = $this->db->insert('tbl06_penyimpanan_berkas',$params);
                if($cekCommand){
                    $this->db->from('tbl06_penyimpanan_berkas');
                    ...
                    if($cekQuery->num_rows() > 0){
                        ...
                        $x['content'] = $this->load->view('penyimpanan_berkas/template_entry',$y,true);
                        $this->load->view('template/theme',$x);         
                    }else{
                ...
                $this->db->from('tbl06_penyimpanan_berkas');
                $this->db->where('idx',$id_penyimpanan_berkas);
                $cekQuery = $this->db->get();
                if($cekQuery->num_rows() > 0){
                    $res = $cekQuery->row_array();
                    $y['idx'] = $res['idx'];
    ...
    function getPasien(){
        ...
        SELECT id_daftar FROM tbl06_penyimpanan_berkas_item WHERE id_penyimpanan_berkas NOT IN(
        SELECT id_penyimpanan_berkas FROM tbl06_penyimpanan_berkas_batal)) ";
        if(isset($_POST['sLike'])){
            $offset = 0;
            $sLike = $this->db->escape_str($this->input->post('sLike',true));
        ...
        $SQL = "SELECT * FROM tbl06_status_kembali_item a $condition ORDER BY idx DESC";

        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target']      = 'tbody#getPasien';
        $config['uri_segment']  = $this->uri_segment;
        $config['base_url']    = base_url().'mr_dokumen.php/penyimpanan_berkas/getPasien';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $limit;
        $this->ajax_page->initialize($config);
        ...
        $x['offset'] = $offset;
        $x['SQL'] = $this->db->query("$SQL LIMIT $offset,$limit");
        $this->load->view('penyimpanan_berkas/getDataKunjungan',$x);
    }
    function getPengembalianStatus(){
        $id_penyimpanan_berkas = $this->input->post('id_penyimpanan_berkas',true);
        $SQL = "SELECT * FROM tbl06_penyimpanan_berkas_item 
        WHERE id_penyimpanan_berkas = '$id_penyimpanan_berkas' ORDER BY idx DESC";
        $x['SQL'] = $this->db->query("$SQL");
        $this->load->view('penyimpanan_berkas/getItem',$x);
    }
    function cekRekap(){
        $id_penyimpanan_berkas = $this->input->post('id_penyimpanan_berkas',true);
        $SQL = "SELECT * FROM tbl06_penyimpanan_berkas_item 
        WHERE id_penyimpanan_berkas = '$id_penyimpanan_berkas'";
    ...
    function simpanItem(){
        ...
        if($ses_state){
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['id_penyimpanan_berkas'])){
                    if($_POST['id_penyimpanan_berkas'] == ""){
                        $response['code'] = 401;
                        ...
                        $this->db->where('id_penyimpanan_berkas',$params['id_penyimpanan_berkas']); 
                        $this->db->where('id_daftar',$params['id_daftar']); 
                        $cekInsert = $this->db->get('tbl06_penyimpanan_berkas_item'); 
                        if ($cekInsert->num_rows() > 0) {
                            $response['code'] = 402;
                            $response['message'] = "Ops. Data telah ada. Silahkan masukan data lainnya";
                        }else{
                            $cekCommand = $this->db->insert('tbl06_penyimpanan_berkas_item',$params); 
                            if($cekCommand){
                                $response['code'] = 200;
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->delete('tbl06_penyimpanan_berkas_item'); 
                        if($cekCommand){
                            $response['code'] = 200;
                ...
                if(isset($_GET['kode'])){
                    if($_GET['kode'] == ""){
                        ...
                        if($cekCommand->num_rows() > 0){
                            ...
                            $this->load->view('penyimpanan_berkas/print_preview',$x);
                        }else{
    ...
    function get_print_data(){
        $this->db->from('tbl06_penyimpanan_berkas_item'); 
        $this->db->where('id_penyimpanan_berkas',$_POST['kode']); 
        $x['SQLItem'] = $this->db->get(); 
        $this->load->view('penyimpanan_berkas/get_print_data',$x);
    }
    function hapus(){
        $ses_state = $this->users_model->cek_session_id();
        if($ses_state){
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $params = array(
                            'id_penyimpanan_berkas' => $_POST['idx'],
                            'userID' => $this->session->userdata('get_uid')
                        );
                        $cekCommand = $this->db->insert('tbl06_penyimpanan_berkas_batal',$params); 
                        if($cekCommand){
                            $response['code'] = 200;
