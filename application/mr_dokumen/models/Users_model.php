        ...
        function cek_user($NRP=null,$userPasw=null){
            if($NRP==null) {
				...
				if($query->num_rows() > 0){
                    ...
                    $queUsersAdmin = $this->db->get('tbl01_users_mr_dokumen');
					if($queUsersAdmin->num_rows() > 0){
                        ...
                        $response['error'] = false;
                        $response['message'] = "Ops. User anda tidak terdaftar sebagai Operator MR Dokumen!\nForm ini hanya dapat di akses oleh Operator MR Dokumen.\nUntuk informasi silahkan hubungi Super Administrator";
                    }
				}else{
					$response['error'] = false;
