<style>
    ...
    .widget-report .widget-user-header{
        padding: 5px 10px;
        alosder-top-right-radius: 3px;
        alosder-top-left-radius: 3px;
    }
    tr.item td:not(.desk){
        text-align: right;
    }
                ...
                <div class="box-body">
                    <form id="form1" role="form" onsubmit="return false">
                        ...
                        <div class="input-group-sm btn-block">
                            ...
                            <select id="ranap" class="form-control">
                                <option value="ALL">Semua Ruang</option>
                                <?php foreach ($datRanap->result_array() as $x): ?>
        ...
        <div class="col-md-10">
            <div class="box box-success">
                <div class="box-body table-responsive">
                    <table class="table table-alosdered">
                        <thead>
                            <tr>
                                <th rowspan="3" style="vertical-align: middle;">Ruang Ranap</th>
                                <th colspan="24" style="text-align: center;">Jumlah Lama Dirawat (D) - Jumlah Pasien Keluar (K)</th>
                        ...
                        <tbody id="getdata">
                            <tr><td colspan="26">Data masih kosong</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
$(document).ready(function () { 
    ...
    $('#btnKeyword').click(function(){
        ...
        b = $('#ranap').val();
        $.ajax({
            url         : "<?php echo base_url().'mr_dokumen.php/sirs/ranap/alos/getReport' ?>",
            type        : "POST",
            data        : {Tahun:a,ranap:b},
            beforeSend  : function(){
                $('#getdata').html("<tr><td colspan=26><i class='fa fa-spin fa-refresh'></i> Loading... Please wait</td></tr>");
            },
            success     : function(data){
                $('#getdata').html(data);
            },
            error       : function(jqXHR,ajaxOption,errorThrown){
                $('#getdata').html('<tr><td colspan=26>Data tidak ditemukan</td></tr>');
    ...
    $('#btnCetak').click(function(){
        a = $('#Tahun').val();
        b = $('#ranap').val();
        var url = '<?php echo base_url().'mr_dokumen.php/sirs/ranap/alos/cetak?thn=' ?>'+a+'&ranap='+b;
        openInNewTab(url);        
    });
});
