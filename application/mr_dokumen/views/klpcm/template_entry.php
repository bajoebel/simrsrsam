...
<section class="content container-fluid">
    <div class="alert alert-danger alert-dismissible">
        ...
        Data hasil pemeriksaan KetidakLengkapan Pengisian Catatan Medis (KLPCM) yang tampil pada tabel pencarian merupakan dokumen yang telah dientrykan pada penerimaan status kembali. Anda dapat mengedit data dengan menekan tombol <b>Edit</b> pada halaman utama. Jangan merefresh browser. Tindakan ini akan mengakibatkan system menggenerate kode rekap baru.    
    </div>
                        ...
                        <h4>
                            <table id="titleRekap">
                                <tr>
                                    ...
                                    <td class="w10">:</td>
                                    <td>
                <input readonly="" type="text" class="form-control" name="txtIdKlpcm" id="txtIdKlpcm" value="<?php echo $idx ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td>Tgl.Rekap</td>
...
$(document).ready(function () { 
    ...
    var id_klpcm = '<?php echo $idx ?>';
    $('#btnKembali').click(function(){
        var url = '<?php echo base_url().'mr_dokumen.php/klpcm' ?>';
        window.location.href = url;    
    });
    $('#btnRefresh').click(function(){
        getStatusKembali(id_klpcm); 
    });
    $('#btnCetak').click(function(){
        var a = $('#txtIdKlpcm').val();
        if(a == ""){
            alert("Kode rekap tidak boleh kosong");
        }else{
            $.ajax({
                url         : "<?php echo base_url().'mr_dokumen.php/klpcm/cekRekap' ?>",
                type        : "POST",
                data        : {id_klpcm:a},
                dataType    : "JSON",
                success : function(data){
                    if(data.error){
                        ...
                        var url = '<?php echo base_url().'mr_dokumen.php/klpcm/cetak?kode=' ?>'+a;
                        window.open(url);  
                    }
                },
    ...
    $('#txtNama').keypress(function(ev){
        ...
        if (keycode == '13') {
            ...
            if(a == ""){
                ...
                $.ajax({
                    url         : "<?php echo base_url().'mr_dokumen.php/klpcm/getPasien' ?>",
                    type        : "POST",
                    data        : {sLike:a},
                    beforeSend  : function(){
...
function getStatusKembali(a){
    $.ajax({
        url         : "<?php echo base_url().'mr_dokumen.php/klpcm/getPengembalianStatus' ?>",
        type        : "POST",
        data        : {id_klpcm:a},
        beforeSend  : function(){
...
function simpanItemStatus(){
    var postData = {}
        postData["id_klpcm"] = $('#txtIdKlpcm').val();
    ...
    if(postData["id_klpcm"]==""){
        ...
        $.ajax({
            url         : "<?php echo base_url().'mr_dokumen.php/klpcm/simpanItem' ?>",
            type        : "POST",
            data        : postData,
            dataType    : "JSON",
            success     : function(data){
                $('#divCariPasien').hide();
                if(data.code==200){
                    getStatusKembali(postData["id_klpcm"]);
                    $('#txtNoRegRS').val('');
                    $('#txtNoMR').val('');
                    $('#txtNama').val('');
                    $('#txtIdRuang').val('');
...
function hapusItem(a,b){
    ...
    if(x){
        $.ajax({
            url         : "<?php echo base_url().'mr_dokumen.php/klpcm/hapusItem' ?>",
            type        : "POST",
            data        : {idx:a},
            dataType    : "JSON",
            success : function(data){
