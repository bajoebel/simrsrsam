...
<section class="content container-fluid">
    <div class="alert alert-danger alert-dismissible">
        ...
        Data penyimpanan berkas yang tampil pada tabel pencarian merupakan berkas yang telah dientrykan oleh petugas pengembalian berkas. Anda dapat mengedit data dengan menekan tombol <b>Edit</b> pada halaman utama. Jangan merefresh browser. Tindakan ini akan mengakibatkan system menggenerate kode rekap baru.    
    </div>
                        ...
                        <h4>
                            <table id="titleRekap">
                                <tr>
                                    ...
                                    <td class="w10">:</td>
                                    <td>
                <input readonly="" type="text" class="form-control" name="txtIdpenyimpanan_berkas" id="txtIdpenyimpanan_berkas" value="<?php echo $idx ?>">
                                    </td>
                                </tr>
                                <tr>
                                    <td>Tgl.Rekap</td>
...
$(document).ready(function () { 
    var id_penyimpanan_berkas = '<?php echo $idx ?>';
    getStatusKembali(id_penyimpanan_berkas);

    $('input').focus(function(){return this.select();});

    $('#txtTglProses').val('<?php echo date('d/m/Y') ?>');
    $('#btnKembali').click(function(){
        var url = '<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas' ?>';
    ...
    $('#btnCetak').click(function(){
        var a = $('#txtIdpenyimpanan_berkas').val();
        if(a == ""){
            alert("Kode rekap tidak boleh kosong");
        }else{
            $.ajax({
                url         : "<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/cekRekap' ?>",
                type        : "POST",
                data        : {id_penyimpanan_berkas:a},
                dataType    : "JSON",
                success : function(data){
                    if(data.error){
                        alert(data.message);
                    }else{
                        var url = '<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/cetak?kode=' ?>'+a;
                        window.open(url);  
                    }
                },
    ...
    $('#txtNama').keypress(function(ev){
        ...
        if (keycode == '13') {
            ...
            if(a == ""){
                ...
                $.ajax({
                    url         : "<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/getPasien' ?>",
                    type        : "POST",
                    data        : {sLike:a},
                    beforeSend  : function(){
...
function getStatusKembali(a){
    $.ajax({
        url         : "<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/getPengembalianStatus' ?>",
        type        : "POST",
        data        : {id_penyimpanan_berkas:a},
        beforeSend  : function(){
...
var postData = {}
    postData["id_penyimpanan_berkas"] = $('#txtIdpenyimpanan_berkas').val();
    ...
    if(postData["id_penyimpanan_berkas"]==""){
        ...
        $.ajax({
            url         : "<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/simpanItem' ?>",
            type        : "POST",
            data        : $.param( postData ),
            dataType    : "JSON",
            success     : function(data){
                $('#divCariPasien').hide();
                if(data.code==200){
                    getStatusKembali(postData["id_penyimpanan_berkas"]);
                    $('#txtNoRegRS').val('');
                    $('#txtNoMR').val('');
                    $('#txtNama').val('');
                    $('#txtIdRuang').val('');
...
function hapusItem(a,b){
    ...
    if(x){
        $.ajax({
            url         : "<?php echo base_url().'mr_dokumen.php/penyimpanan_berkas/hapusItem' ?>",
            type        : "POST",
            data        : {idx:a},
            dataType    : "JSON",
            success : function(data){
