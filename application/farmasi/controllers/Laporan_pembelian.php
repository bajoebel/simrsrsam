...
class laporan_pembelian extends CI_Controller {
    ...
    public function index(){
        ...
        if ($ses_state) {
            ...
            $y['index_menu'] = 9;
            $y['contentTitle'] = "Laporan Pembelian";
            
            $y['datlokasi'] = $this->db->get('tbl04_lokasi');
            $x['content'] = $this->load->view('laporan/laporan_pembelian',$y,true);
    ...
    function cetak_lap(){
        ...
        if($cek->num_rows() > 0){
            $res = $cek->row_array();
                ...
                if($STATUS == 'cetak'){
                    $this->load->view('laporan/print_pembelian',$x);
                }
                if($STATUS == 'export'){
    ...
    function get_data_lap(){
        ...
        $SQL = "SELECT a.`KDBL`,a.`DTBL`,a.`NOFAKTUR`,b.`NMSUPPLIER`,a.`PEMBAYARAN`,a.TOTPPN,a.KETBL,a.`GRANDTOT` FROM `tbl04_pembelian` a
        LEFT JOIN `tbl04_supplier` b ON a.`KDSUPPLIER`=b.`KDSUPPLIER`
        WHERE a.`KDLOKASI`='$KDLOKASI' AND a.`TGLFAKTUR` >= '". setDateEng($TGL_AWAL)."' AND a.`TGLFAKTUR` <= '". setDateEng($TGL_AKHIR)."' ORDER BY a.`KDBL` ASC";
        //echo $SQL;exit;
        $dataPreview = $this->db->query("$SQL");
        //print_r($dataPreview);exit;
        $x['dataPreview'] = $dataPreview;

        $this->load->view('laporan/get_pembelian',$x); 
    }
