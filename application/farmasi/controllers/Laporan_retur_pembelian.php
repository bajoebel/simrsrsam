...
class laporan_retur_pembelian extends CI_Controller {
    ...
    public function index(){
        ...
        if ($ses_state) {
            ...
            $y['index_menu'] = 9;
            $y['contentTitle'] = "Laporan Retur Pembelian";
            
            $y['datlokasi'] = $this->db->get('tbl04_lokasi');
            $x['content'] = $this->load->view('laporan/laporan_retur_pembelian',$y,true);
    ...
    function cetak_lap(){
        ...
        if($cek->num_rows() > 0){
            $res = $cek->row_array();
                ...
                if($STATUS == 'cetak'){
                    $this->load->view('laporan/print_retur_pembelian',$x);
                }
                if($STATUS == 'export'){
    ...
    function get_data_lap(){
        ...
        $SQL = "SELECT a.`KDBL_RET`,a.`DTBL_RET`,a.`ALASAN`,b.`NOFAKTUR`,c.`NMSUPPLIER`
        FROM `tbl04_pembelian_batal` a
        JOIN `tbl04_pembelian` b ON a.`KDBL`=b.`KDBL`
        JOIN `tbl04_supplier` c ON b.`KDSUPPLIER`=c.`KDSUPPLIER`
        WHERE a.`DTBL_RET` >= '". setDateEng($TGL_AWAL)."' AND a.`DTBL_RET` <= '". setDateEng($TGL_AKHIR)."' ORDER BY a.`KDBL_RET` ASC";
        //echo $SQL;exit;
        $dataPreview = $this->db->query("$SQL");
        //print_r($dataPreview);exit;
        $x['dataPreview'] = $dataPreview;

        $this->load->view('laporan/get_retur_pembelian',$x); 
    }
