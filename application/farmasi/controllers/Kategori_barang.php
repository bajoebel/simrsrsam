    ...
    class kategori_barang extends CI_Controller{
        ...
        function index(){
            ...
            if($ses_state){
                ...
                $y['index_menu'] = 2;
                $y['contentTitle'] = "Kategori Barang";
                $x['content'] = $this->load->view('kategori_barang/template_table',$y,true);
        ...
        function getView(){
            ...
            if(isset($_POST['sLike'])){
                ...
                $condition .= "WHERE NMKTBRG LIKE '%$sLike%'";
            ...
            $SQL = "SELECT * FROM tbl04_kategori_barang $condition ORDER BY KDKTBRG";
            $SQL_Count = $this->db->query("$SQL");
            $totalRec = $SQL_Count->num_rows();
            $config['target']      = 'tbody#getdata';
            $config['uri_segment']  = $this->uri_segment;
            $config['base_url']    = base_url().'farmasi.php/kategori_barang/getView';
            ...
            $this->load->view('kategori_barang/getdata',$x);
        }
        function simpan(){
            $ses_state = $this->users_model->cek_session_id();
            if($ses_state){
                if($_SERVER['REQUEST_METHOD'] == "POST"){
                    if(isset($_POST['KDKTBRG'])){
                        if($_POST['KDKTBRG'] == ""){
                            $params = array(
                                'NMKTBRG' => $this->input->post('NMKTBRG',TRUE)
                            );
                            $cekCommand = $this->db->insert('tbl04_kategori_barang',$params); 
                            if($cekCommand){
                                $response['code'] = 200;
                            ...
                            $this->db->where('KDKTBRG',  $this->input->post('KDKTBRG',TRUE));
                            $cekCommand = $this->db->update('tbl04_kategori_barang',$params); 
                            if($cekCommand){
                                $response['code'] = 201;
                ...
                if($_SERVER['REQUEST_METHOD'] == "POST"){
                    if(isset($_POST['KDKTBRG'])){
                        if($_POST['KDKTBRG'] == ""){
                            ...
                            $queAvailable = "SELECT * FROM tbl04_barang WHERE KDKTBRG = '$_POST[KDKTBRG]' LIMIT 1";
                            ...
                            if ($cekAvailable->num_rows() > 0) {
                                ...
                                $queCommand = "DELETE FROM tbl04_kategori_barang WHERE KDKTBRG = '$_POST[KDKTBRG]'";
