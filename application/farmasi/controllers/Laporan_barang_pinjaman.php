...
class laporan_barang_pinjaman extends CI_Controller {
    ...
    public function index(){
        ...
        if ($ses_state) {
            ...
            $y['index_menu'] = 9;
            $y['contentTitle'] = "Laporan Barang Pinjaman";
            
            $y['datlokasi'] = $this->db->get('tbl04_lokasi');
            $x['content'] = $this->load->view('laporan/laporan_barang_masuk_pinjaman',$y,true);
    ...
    function cetak_lap(){
        ...
        if($cek->num_rows() > 0){
            $res = $cek->row_array();
                ...
                if($STATUS == 'cetak'){
                    $this->load->view('laporan/print_barang_masuk_pinjaman',$x);
                }
                if($STATUS == 'export'){
    ...
    function get_data_lap(){
        ...
        $SQL = "SELECT a.`KDBMK`,a.`DTBMK`,a.`NOFAKTUR`,a.`NMREKANAN` FROM `tbl04_barang_masuk_khusus` a
        WHERE a.`KDLOKASI`='$KDLOKASI' AND a.`TGLFAKTUR` >= '". setDateEng($TGL_AWAL)."' 
        AND a.`TGLFAKTUR` <= '". setDateEng($TGL_AKHIR)."' ORDER BY a.`KDBMK` ASC";
        //echo $SQL;exit;
        $dataPreview = $this->db->query("$SQL");
        //print_r($dataPreview);exit;
        $x['dataPreview'] = $dataPreview;

        $this->load->view('laporan/get_pinjaman',$x); 
    }
