...
class Ajax_page
{
	...
	var $kdlokasi           = ''; // The page we are linking to
	...
	var $last_tag_open		= '&nbsp;';
	var $last_tag_close		= '';
	var $cur_tag_open		= '&nbsp;<b class="btn disabled">';
	var $cur_tag_close		= '</b>';
	var $next_tag_open		= '&nbsp;';
	var $next_tag_close		= '&nbsp;';
	var $prev_tag_open		= '&nbsp;';
	...
	function create_links()
	{
		...
		if ($this->show_count) {
			...
			$info = 'Showing ' . number_format(($curr_offset + 1), 0, ',', '.') . ' to ';

			if (($curr_offset + $this->per_page) < ($this->total_rows - 1))
				$info .= number_format($curr_offset + $this->per_page, 0, ',', '.');
			else
				$info .= number_format($this->total_rows, 0, ',', '.');

			$info .= ' of ' . number_format($this->total_rows, 0, ',', '.') . ' | ';
		...
		$token_input = $CI->security->get_csrf_token_name();
?>
		<script>
			function getData(page, kdlokasi = null) {
				var formItems = {};
				if (kdlokasi == null) {
					formItems['page'] = page;
				} else {
					formItems['page'] = page;
					formItems['kLok'] = kdlokasi;
				}	
				formItems['<?= $token_input ?>'] = $('#csrf').val();
				var url = "<?php echo $this->base_url; ?>" + page;
				$.ajax({
					url: "<?php echo $this->base_url; ?>" + page,
					type: "POST",
					data: formItems,
					beforeSend: function() {
						$('<?php echo $this->target; ?>').html("");
...
<?php
	...
	function getAJAXlink($count, $text)
	{
		...
		if ($kdlokasi == '') {
			return '<a class="btn" href="javascript:void(0);"' . $this->anchor_class . ' onclick="' . $this->link_func . '(' . $pageCount . ')">' . $text . '</a>';
		} else {
			return '<a class="btn" href="javascript:void(0);"' . $this->anchor_class . ' onclick="' . $this->link_func . '(' . $pageCount . ',' . $kdlokasi . ')">' . $text . '</a>';
		}
	}
}
