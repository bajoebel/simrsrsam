...
class Login extends CI_Controller {
    ...
    public function index(){        
        ...
        if($ses_state){
            redirect(base_url().'administrator.php/dashboard');
        }else{
    ...
    public function cek(){
        if($_SERVER['REQUEST_METHOD'] == "POST"){
            ...
            if($get_res['error']){
                ...
                if($get_res){
                    $params = array(
                        'get_uid' => $uid,'level'=>$level
                    );
                    $this->session->set_userdata($params);
                    $response['error'] = true;
                    $response['message'] = base_url().'administrator.php/dashboard';
                }else{
    ...
    public function logout(){
        ...
        $this->session->unset_userdata('get_username');
        redirect(base_url().'administrator.php/login');
    }
