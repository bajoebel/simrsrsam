<section class="sidebar">
    <div class="user-panel">
        <div class="pull-left image">
            <img src="data:image/png;base64," class="img-circle" alt="User Image">
        </div>
        <div class="pull-left info">
            <p><?php echo getUserLogin() ?></p>
            <a href="#"><i class="fa fa-circle text-success"></i> <?php echo getTimeUserLogin() ?></a>
        </div>
    </div>

    <ul class="sidebar-menu" data-widget="tree" id="menu">
        <li class="header"><?php echo getAppName() ?></li>
        <li class="<?php echo $nav_1; ?>">
            <a href="<?php echo base_url() . 'administrator.php/dashboard' ?>">
                <i class="fa fa-home"></i> <span>Home</span></a>
        </li>
        <li class="treeview <?php echo $nav_2; ?>">
            <a href="#"><i class="fa fa-list"></i> <span>Master Data</span>
                <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                </span>
            </a>
            <ul class="treeview-menu">
                <li><a href="<?php echo base_url() . 'administrator.php/suku' ?>">
                        <i class="fa fa-circle-o text-red"></i>Suku</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/bahasa' ?>">
                        <i class="fa fa-circle-o text-red"></i>Bahasa</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/cara_bayar' ?>">
                        <i class="fa fa-circle-o text-red"></i>Cara Bayar</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/kelas_layanan' ?>">
                        <i class="fa fa-circle-o text-red"></i>Kelas Layanan</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/jenis_pelayanan' ?>">
                        <i class="fa fa-circle-o text-red"></i>Tindakan Pelayanan</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/group_layanan' ?>">
                        <i class="fa fa-circle-o text-red"></i>Group Layanan</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/ruang_pelayanan' ?>">
                        <i class="fa fa-circle-o text-red"></i>Ruang Pelayanan</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/keadaan_keluar' ?>">
                        <i class="fa fa-circle-o text-red"></i>Keadaan Keluar</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/cara_keluar' ?>">
                        <i class="fa fa-circle-o text-red"></i>Cara Keluar</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/profesi' ?>">
                        <i class="fa fa-circle-o text-red"></i>Profesi</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/pegawai' ?>">
                        <i class="fa fa-circle-o text-red"></i>Pegawai</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/app_users' ?>">
                        <i class="fa fa-circle-o text-red"></i>App Users</a></li>
            </ul>
        </li>
        <li class="treeview <?php echo $nav_3; ?>">
            <a href="#"><i class="fa fa-list"></i> <span>Master Tarif</span>
                <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                </span>
            </a>
            <ul class="treeview-menu">
                <li><a href="<?php echo base_url() . 'administrator.php/kategori_tarif' ?>">
                        <i class="fa fa-circle-o text-red"></i>Kategori Tarif</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/tarif_layanan' ?>">
                        <i class="fa fa-circle-o text-red"></i>Tarif Layanan</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/mapping_tarif' ?>">
                        <i class="fa fa-circle-o text-red"></i>Tarif Ruangan</a></li>
            </ul>
        </li>
        <li class="treeview <?php echo $nav_4; ?>">
            <a href="#"><i class="fa fa-list"></i> <span>Master Data Farmasi</span>
                <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                </span>
            </a>
            <ul class="treeview-menu">
                <li><a href="<?php echo base_url() . 'administrator.php/group_lokasi' ?>">
                        <i class="fa fa-circle-o text-red"></i>Group Lokasi</a></li>
                <li><a href="<?php echo base_url() . 'administrator.php/lokasi' ?>">
                        <i class="fa fa-circle-o text-red"></i>Lokasi Obat</a></li>
            </ul>
        </li>

        <li class="<?php echo $nav_5; ?>">
            <a href="<?php echo base_url() . 'administrator.php/dtd' ?>">
                <i class="fa fa-list"></i> <span>Kelompok DTD KIP/10</span></a>
        </li>
    </ul>
</section>