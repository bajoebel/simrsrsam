    ...
    class kecamatan extends CI_Controller{
        ...
        function index(){
            ...
            if($ses_state){
                ...
                $y['contentTitle'] = "Kecamatan";
                $y['datProvinsi'] = $this->db->get('tbl01_provinsi'); 
                $y['datKabKota'] = $this->db->get('tbl01_kab_kota'); 

                $x['content'] = $this->load->view('kecamatan/template_table',$y,true);
        ...
        function getView(){
            ...
            if(isset($_POST['sLike'])){
                ...
                $condition .= "WHERE nama_provinsi LIKE '%$sLike%' OR nama_kab_kota LIKE '%$sLike%' OR nama_kecamatan LIKE '%$sLike%'";
            ...
            $SQL = "SELECT * FROM tbl01_kecamatan $condition ORDER BY idx";
            $SQL_Count = $this->db->query("$SQL");
            $totalRec = $SQL_Count->num_rows();
            $config['target']      = 'tbody#getdata';
            $config['uri_segment']  = $this->uri_segment;
            $config['base_url']    = base_url().'mr_registrasi.php/kecamatan/getView';
            ...
            $this->load->view('kecamatan/getdata',$x);
        }
        function simpan(){
            $ses_state = $this->users_model->cek_session_id();
            if($ses_state){
                if($_SERVER['REQUEST_METHOD'] == "POST"){
                    if(
                        ...
                        if($params['idx'] == ""){
                            $cekCommand = $this->db->insert('tbl01_kecamatan',$params); 
                            if($cekCommand){
                                $response['code'] = 200;
                            ...
                            $cekCommand = $this->db->update('tbl01_kecamatan',$params); 
                            if($cekCommand){
                                $response['code'] = 201;
                ...
                if($_SERVER['REQUEST_METHOD'] == "POST"){
                    if(isset($_POST['idx'])){
                        if($_POST['idx'] == ""){
                            ...
                            $cekCommand = $this->db->query("DELETE FROM tbl01_kecamatan WHERE idx = '$_POST[idx]'"); 
                            if($cekCommand){
                                $response['code'] = 200;
