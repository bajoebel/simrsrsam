...
class Provinsi extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['contentTitle'] = "Provinsi";
            $x['content'] = $this->load->view('provinsi/template_table',$y,true);
    ...
    function getView(){
        ...
        if(isset($_POST['sLike'])){
            ...
            $condition .= "WHERE nama_provinsi LIKE '%$sLike%'";
        ...
        $SQL = "SELECT * FROM tbl01_provinsi $condition ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target']      = 'tbody#getdata';
        $config['uri_segment']  = $this->uri_segment;
        $config['base_url']    = base_url().'mr_registrasi.php/provinsi/getView';
        ...
        $this->load->view('provinsi/getdata',$x);
    }
    function simpan(){
        $ses_state = $this->users_model->cek_session_id();
        if($ses_state){
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->insert('tbl01_provinsi',$params); 
                        if($cekCommand){
                            $response['code'] = 200;
                        ...
                        $cekCommand = $this->db->update('tbl01_provinsi',$params); 
                        if($cekCommand){
                            $response['code'] = 201;
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->query("DELETE FROM tbl01_provinsi WHERE idx = '$_POST[idx]'"); 
                        if($cekCommand){
                            $response['code'] = 200;
