...
class Negara extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['contentTitle'] = "Negara";
            $x['content'] = $this->load->view('negara/template_table',$y,true);
    ...
    function getView(){
        ...
        if(isset($_POST['sLike'])){
            ...
            $condition .= "WHERE nama_negara LIKE '%$sLike%'";
        ...
        $SQL = "SELECT * FROM tbl01_negara $condition ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target']      = 'tbody#getdata';
        $config['uri_segment']  = $this->uri_segment;
        $config['base_url']    = base_url().'mr_registrasi.php/negara/getView';
        ...
        $this->load->view('negara/getdata',$x);
    }
    function simpan(){
        $ses_state = $this->users_model->cek_session_id();
        if($ses_state){
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->insert('tbl01_negara',$params); 
                        if($cekCommand){
                            $response['code'] = 200;
                        ...
                        $cekCommand = $this->db->update('tbl01_negara',$params); 
                        if($cekCommand){
                            $response['code'] = 201;
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->query("DELETE FROM tbl01_negara WHERE idx = '$_POST[idx]'"); 
                        if($cekCommand){
                            $response['code'] = 200;
