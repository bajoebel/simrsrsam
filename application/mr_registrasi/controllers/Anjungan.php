...
class Anjungan extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){      
            ...
            $y['contentTitle'] = "Pendaftaran Pasien Anjungan Mandiri";
            $x['content'] = $this->load->view('registrasi/v_anjunganmandiri',$y,true);
    ...
    function data(){
            ...
            $limit=50;
            $data=array(
                'data'=>$this->pendaftaran_model->getPasienAnjungan($poly,$tgl,$filter,$q,$start,$limit)
            );
            // print_r($data);exit;
            $jmldata=$this->pendaftaran_model->countPasienAnjungan($poly,$tgl,$filter,$q);
            $tabel=$this->load->view('registrasi/v_data_pendaftaran_mandiri',$data, true);
            $list=array(
                'start'     => $start,
                'row_count' => $jmldata,
                'limit'     => $limit,
