...
class kelas extends CI_Controller
{
    ...
    function index()
    {
        ...
        if (isAdminLogin()) {
            ...
            $y['contentTitle'] = "Kelas Ruang Pelayanan";
            $x['content'] = $this->load->view('kelas/template_table', $y, true);
    ...
    function getView()
    {
        ...
        $SQL = "SELECT * FROM tbl_kelas $condition ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target'] = 'tbody#getdata';
        $config['uri_segment'] = $this->uri_segment;
        $config['base_url'] = base_url() . 'kelas/getView';
        ...
        $this->load->view('kelas/getdata', $x);
    }
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                if (
                    ...
                    if ($params['idx'] == "") {
                        ...
                        $cekCommand = $this->db->update('tbl_kelas', $params);
                        if ($cekCommand) {
                            $response['code'] = 201;
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                if (isset($_POST['idx'])) {
                    if ($_POST['idx'] == "") {
                        ...
                        $cekCommand = $this->db->query("DELETE FROM tbl_kelas WHERE idx = '$_POST[idx]'");
                        if ($cekCommand) {
                            $response['code'] = 200;
