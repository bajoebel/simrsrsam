...
<script type="text/javascript">
	...
	function cetaktracer(a){
		var kode = a;
		var url = '<?php echo base_url() .'pendaftaran/cetakTracerOnlineBaru/'; ?>'+kode;
		window.open(url);
	}
	function daftaronline(a){
		var kode = a;
		location.href = "<?php echo base_url() .'pendaftaran/get_booking_baru/'; ?>"+kode;
	}
	jQuery(function($) {
		...
		$('#cek').click(function(){
			...
			$.ajax({ 
				type	: 'POST',
				url		: "<?php echo base_url().'pendaftaran/get_listbooking_baru'; ?>", 
				...
				success	: function(data){ 
						...
						for(a = 0; a < jumlah; a++){
							...
							if(data.response[a].jns_kelamin=="L"){
								var gender = "LAKI-LAKI";
							}
							if(data.response[a].jns_kelamin=="P"){
								var gender = "PEREMPUAN";
							}
								buatTabel += "<tr>"
								+ "<td>" + (a+1) + "</td>"
								+ "<td>" + data.response[a].kode_booking + "</td>"
								+ "<td>" + data.response[a].noktp + "</td>"
								+ "<td>" + data.response[a].nama + "</td>"
								+ "<td>" + data.response[a].t_lahir + "/" + data.response[a].tgl_lahir + "</td>"
								+ "<td>" + gender + "</td>"
								+ "<td>" + data.response[a].alamat + "</td>"
