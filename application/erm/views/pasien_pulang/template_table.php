...
$(document).ready(function () { 
    ...
    $('#btnKembali').click(function(){
        var url = '<?php echo base_url().'nota_tagihan.php/pasien_pulang' ?>';
        window.location.href = url;
    });
    $('#btnTambah').click(function(){
        var a = '<?php echo $ruangID ?>';
        var url = '<?php echo base_url().'nota_tagihan.php/pasien_pulang/tambah?kLok=' ?>'+a;
        window.location.href = url;
    });
    ...
    $('#keywords').keypress(function(ev){
        ...
        if (keycode == '13') {
            $.ajax({
                url         : "<?php echo base_url().'nota_tagihan.php/pasien_pulang/getView' ?>",
                type        : "POST",
...
function batalPulang(a){
    ...
    if(x){
        if(a==""){
            ...
            $.ajax({
                url     : "<?php echo base_url().'nota_tagihan.php/pasien_pulang/batalPulang' ?>",
                type    : "POST",
                data    : {idx:a},
                dataType: "JSON",
                success : function(data){
