...
<head>
    ...
    <style type="text/css">
        ...
        .unchecked{
            list-style-image: url('<?= base_url() ."assets/images/unchecked.png" ?>');
        }
    </style>
</head>
<body>
            ...
            //echo $id_penunjang;
                if($id_penunjang==49){
                    ...
                    */ 
                            ...
                            foreach ($tindakan as $p ) {
                                ...
                                if($jenispemeriksaan!=$p->jenispemeriksaan){
                                    if($i>0) {
                                        ...
                                        if($i%3==0)  {
                                            $t.="</td></tr><td valign='top'><b>".$p->jenispemeriksaan ."</b>" ;
                                            if($p->semua_variabel==0) $t.="<ul><li>".$p->subjenispemeriksaan  ."</li>";  
                                        }
                                        else {
                                            $t.="</td><td valign='top'><b>".$p->jenispemeriksaan ."</b>";
                                            if($p->semua_variabel==0) $t.="<ul><li>".$p->subjenispemeriksaan ."</li>";  
                                    ...
                                    if($p->semua_variabel==0) $t.="<li>".$p->subjenispemeriksaan ."</li>";
                                }
                     ...
                     */
                    // print_r($permintaan);
                    $jenis = $this->nota_model->getPilihanJenisPemeriksaan($id_penunjang,$permintaan->idx);
                    ?>
                    <table>
                        <tr>
                        ...
                        <?php
                            ...
                            foreach ($jenis as $j ) {
                                if($jenis_pemeriksaan!=$j->jenis_pemeriksaan){
                                    ...
                                    $sj=explode(',',$j->sub_jenispemeriksaan);
                                        foreach ($sj as $s ) {
                                            if($j->subjenispemeriksaan==trim($s)) {
                                                $arsj[]=$s;
                                            }
                                            else {
                                                $arsj[]="<strike>".$s."</strike>";
                                            }
                                            // echo "<br>".$j->subjenispemeriksaan;
                                        }
                                    if($j->idx==$j->idjenispemeriksaanpilih) {
                                        
                                        $li .= "<li type='square' class='checked'>" .$j->jenis_pemeriksaan ." : " .implode($arsj);
                                    }
                                    
                                    else $li.="<li type='square' class='unchecked'>" .$j->jenis_pemeriksaan ." : " .$j->sub_jenispemeriksaan;
                                }else{
                        ...
                        <td > : <img src="<?= str_replace("[removed]","data:image/png;base64,",$permintaan->lokasi_jaringan); ?>" alt=""></td>
                        </tr>
                        <tr>
