...
class nota_tagihan extends CI_Controller
{
                ...
                if ($this->session->userdata('kdlokasi') && isset($_GET['idx'])) {
                    if ($this->session->userdata('kdlokasi') == "" || $_GET['idx'] == "") {
                        ...
                        if (!empty($y["detail"])) {
                            ...
                            // echo $y['detail']->kode_permintaan;exit;
    ...
    function simpanjadwaloperasi()
    {
        ...
        if ($ses_state) {
            ...
            if (!$arr_res) {
                ...
                if ($arr_res->metadata->code == 200) $status_kirim = 1;
                else $status_kirim = 0;
            }
            $antrean = array(
                'id_daftar'     => $this->input->post('id_daftar'),
                ...
                if($insert_id){
                    foreach ($index as $idp ) {
                        ...
                        foreach ($id_subpemeriksaan as $idsp ) {
                            ...
                            $hasilpost='hasil'.$hasil;
                            $subpemeriksaan=$this->input->post('subpemeriksaan'.$idp.$idsp);
                            $proyeksi="";
                            $tingkatpositif="";
                            $nanah_lendir=0;
                            $bercak_darah=0;
                            ...
                            if($subpemeriksaan=="" || $subpemeriksaan=="-") $subpemeriksaan= $this->input->post('nama_pemeriksaan'.$idp);
                            if($template=="DahakTB"){
                                $tingkatpositif=$this->input->post('tingkatpositif'.$idp.$idsp);
                                $nanah_lendir=$this->input->post('nanah_lendir'.$idp.$idsp);
                                $bercak_darah=$this->input->post('bercak_darah'.$idp.$idsp);
                                $air_liur=$this->input->post('air_liur'.$idp.$idsp);
                                if(empty($nanah_lendir)) $nanah_lendir=0;
                                if(empty($bercak_darah)) $bercak_darah=0;
                                if(empty($air_liur)) $air_liur=0;
                            }else if($template=="Radiologi"){
                            ...
                            $detailhasil[]=array(
                                ...
                                'bercak_darah'=>$bercak_darah,
                                'air_liur'=>$air_liur,
                                'satuan'=>$this->input->post('satuan'.$idp.$idsp),
                                'rujukan_lk'=>$this->input->post('nilai_rujukan_lk'.$idp.$idsp),
                                'rujukan_pr'=>$this->input->post('nilai_rujukan_pr'.$idp.$idsp),
                                'lampiran'=>'',
                                'validator'=>'',
                            );
                        }
    ...
    function hasilpenunjang($nomr)
    {
        ...
        if ($ses_state) {
            $mulai = ($start * $limit) - $limit;
            $row_count = $this->nota_model->countHasilPemeriksaanPenunjang($nomr, $q);
            $data = $this->nota_model->getHasilPemeriksaanPenunjang($nomr, $limit, $mulai, $q);
    ...
    function riwayat_pemakaian_obat($nomr)
    {
        $ses_state = $this->users_model->cek_session_id();
        $start = intval($this->input->get("start"));
        $limit = intval($this->input->get("limit"));
        ...
        if ($ses_state) {
            $mulai = ($start * $limit) - $limit;
            $row_count = $this->nota_model->countRiwayatPemakaianObat($nomr, $q);
            $data = $this->nota_model->getRiwayatPemakaianObat($nomr, $limit, $mulai, $q);
