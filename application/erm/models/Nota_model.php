...
class Nota_model extends CI_Model
{
    ...
    function ws_antrean($request, $url, $token = "")
    {
        //$data = array("name" => "Hagrid", "age" => "36");    
        // echo ONLINE_CALL_BACK.$url;                                                                
        $data_string = json_encode($request);
        $ch = curl_init(ONLINE_CALL_BACK . $url);
        ...
        if (empty($token)) {
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                'X-Username: ' . ONLINE_ID,
                'X-Password: ' . ONLINE_KEY
            ));
        } else {
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                'Content-Type: application/json',
    ...
    function getHasilPemeriksaanByPasien($nomr, $idjenis)
    {
        ...
        $this->db->where('verifikator <>', '');
        $this->db->order_by('reg_unit', 'desc');
        $this->db->order_by('idjenispemeriksaan');
        $this->db->join('tbl03_hasil_pemeriksaan_penunjang_detail b','a.idx=idhasil');
        return $this->db->get('tbl03_hasil_pemeriksaan_penunjang a')->result();
    }
    function getHasilPemeriksaanById($idhasil){
        ...
        ->get('tbl03_hasil_pemeriksaan_penunjang a')->result_array();
    }
    function countHasilPemeriksaanPenunjang($nomr,$q=""){
        return $this->db->where('nomr',$nomr)
            ->group_start()
            ->like('reg_unit',$q)
            ->or_like('nomr',$q)
            ->or_like('nama_pasien',$q)
            ->or_like('ruangpengirim',$q)
            ->or_like('diagnosa',$q)
            ->group_end()
            ->get('tbl03_hasil_pemeriksaan_penunjang')
            ->num_rows();
    }
    function getHasilPemeriksaanPenunjang($nomr,$limit=10,$mulai=0,$q=""){
        return $this->db->select('a.*,SUM(CASE WHEN (b.validasipetugas=0) THEN 1 ELSE 0 END) AS belumvalid')
            ->where('nomr',$nomr)
            ->join('tbl03_hasil_pemeriksaan_penunjang_detail b','a.idx=b.idhasil')
            ->group_start()
            ->like('reg_unit',$q)
            ...
            ->group_by('a.idx')
            ->limit($limit, $mulai)
            ->get('tbl03_hasil_pemeriksaan_penunjang a')
            ->result();
    }
    function getHasil($idhasil){
        return $this->db->where('idhasil',$idhasil)
        ->join('tbl01_esign','kode=verifikator')
        ->get('tbl03_hasil_pemeriksaan_signdata')->row();
    }
    ...
    {
        $this->db->select("*, b.idx,GROUP_CONCAT(sub_jenispemeriksaan SEPARATOR ', ') AS sub_jenispemeriksaan");
    ...
    function getPilihanJenisPemeriksaan($id_penunjang,$permintaanid)
    {
        $this->db->select("*, b.idx,GROUP_CONCAT(sub_jenispemeriksaan SEPARATOR ', ') AS sub_jenispemeriksaan");
        $this->db->where('idpenunjang', $id_penunjang);
        $this->db->join('tbl01_jenis_pemeriksaan_sub a', 'a.idjenispemeriksaan=b.idx', 'left');
        $this->db->join("(SELECT c.id_permintaan,c.idjenispemeriksaan AS idjenispemeriksaanpilih,c.jenispemeriksaan,c.`idsubjenispemeriksaan`,d.`sub_jenispemeriksaan` AS `subjenispemeriksaan` 
        FROM `tbl02_permintaan_tindakan_penunjang` c JOIN `tbl01_jenis_pemeriksaan_sub` d ON c.idsubjenispemeriksaan=d.idx
        WHERE id_permintaan=$permintaanid GROUP BY c.idjenispemeriksaan) as temp","b.idx=temp.idjenispemeriksaanpilih","LEFT");
        $this->db->group_by('b.idx');
        return $this->db->get("tbl01_jenis_pemeriksaan b")->result();
    }
    ...
    function getPermintaanJenisPemeriksaan($idx_pendaftaran)
    {
        ...
        return $this->db->get("tbl02_pemeriksaan_penunjang a")->result();
    }
    ...
    function getVariabelPemeriksaan($id){
        return $this->db->where('id_pemeriksaan',$id)->get("tbl01_variabelpemeriksaan")->result();  
    }

    function permintaanPemeriksaan($idjenis, $idx_pendaftaran = '', $reg_unit = '')
    {
        $this->db->where('c.id_jenis_pemeriksaan', $idjenis);
        if (!empty($idx_pendaftaran)) $this->db->where("c.idx_pendaftaran", $idx_pendaftaran);
        if (!empty($reg_unit)) $this->db->where("c.id_pemeriksaan NOT IN(SELECT idpemeriksaan FROM tbl03_hasil_pemeriksaan_penunjang a 
        JOIN tbl03_hasil_pemeriksaan_penunjang_detail b ON a.idx=b.idhasil WHERE reg_unit='" . $reg_unit . "')");
        $this->db->join('tbl01_variabelpemeriksaan d','c.id_subjenispemeriksaan=d.variabelid');
        $this->db->order_by('c.id_pemeriksaan,c.id_subjenispemeriksaan');
        return $this->db->get('tbl02_pemeriksaan_penunjang c')->result();
    }
    ...
    function getSubPemeriksaan($idjenis){
        return $this->db->where('idjenispemeriksaan',$idjenis)
        ->get('tbl01_jenis_pemeriksaan_sub')->result();
    }
    ...
    function getRiwayatPemakaianObat($nomr, $limit, $start, $q)
    {
        return $this->db->where('a.NOMR',$nomr)
        ->group_start()
        ->like('KDJL',$q)
        ->or_like('DTJL',$q)
        ->or_like('NMRUANGAN',$q)
        ->or_like('NMDOKTER',$q)
        ->group_end()
        ->select("*,(SELECT CONCAT('<ol>',GROUP_CONCAT(CONCAT('<li>',NMBRG,' - ',AP,'</li>') SEPARATOR ''),'</ol>') AS obat FROM `tbl04_penjualan_detail` WHERE KDJL=a.KDJL GROUP BY KDJL) AS obat")
        ->limit($limit, $start)
        ->group_by('KDJL')
        ->order_by('DTJL','DESC')
        ->get('tbl04_penjualan a')->result();
    }
    function countRiwayatPemakaianObat($nomr, $q)
    {
        return $this->db->where('a.NOMR',$nomr)
        ->group_start()
        ->like('KDJL',$q)
        ...
        ->or_like('NMDOKTER',$q)
        ->group_end()
        ->get('tbl04_penjualan a')->num_rows();
    }
    ...
    function getPermintaanTindakan($idpermintaan)
    {
        ...
        $this->db->join('tbl01_jenis_pemeriksaan b','a.idjenispemeriksaan=b.idx');
        $this->db->order_by('idjenispemeriksaan');
        return $this->db->get('tbl02_permintaan_tindakan_penunjang a')->result();
    }
    ...
    function updateBed($id_kamar)
    {
        ...
        if ($row) {
            ...
            $respone = $this->request_online($param, ONLINE_CALL_BACK . "api/updatebed");
            return $respone;
        }
    ...
    function getPemeriksaanByJenis($id_daftar,$idjenis){
        return $this->db->where('idx_pendaftaran',$id_daftar)
        ->where('id_jenis_pemeriksaan',$idjenis)
        ->get('tbl02_pemeriksaan_penunjang')->result();
    }
    ...
    function insertHasilPemeriksaan($data){
        $this->db->insert('tbl03_hasil_pemeriksaan_penunjang',$data);
        return $this->db->insert_id();
    }
    function updateHasilPemeriksaan($data,$idx){
        $this->db->where('idx',$idx);
        $this->db->update('tbl03_hasil_pemeriksaan_penunjang',$data);
    }
}
