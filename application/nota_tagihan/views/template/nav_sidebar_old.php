<section class="sidebar">
    <div class="user-panel">
        <div class="pull-left image">
            <img src="data:image/png;base64," class="img-circle" alt="User Image">
        </div>
        <div class="pull-left info">
            <p><?php echo getUserLogin() ?></p>
            <a href="#"><i class="fa fa-circle text-success"></i> <?php echo getTimeUserLogin() ?></a>
 
        </div>
    </div>
    <ul class="sidebar-menu" data-widget="tree">
        <li class="header"><?php echo getAppName() ?></li>
        <?php 
        $nota_tagihan=$this->nota_model->cekRuang();
        $rajal=$this->nota_model->cekRuang("1");
        $ranap=$this->nota_model->cekRuang("2");
        $penunjang=$this->nota_model->cekRuang("3");
        $igd=$this->nota_model->cekRuang("4");
        ?>
        <li class="<?php echo $nav_1; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/dashboard' ?>">
                <i class="fa fa-home"></i> <span>Home</span></a>
        </li>
        <?php if($nota_tagihan>0) { ?>
        <!--li class="<?php //echo $nav_2; ?>">
            <a href="<?php //echo base_url().'nota_tagihan.php/nota_tagihan' ?>">
                <i class="fa fa-cart-plus"></i> <span>Nota Tagihan</span></a>
        </li-->

        <li class="<?php echo $nav_2; ?> treeview">
          <a href="#">
            <i class="fa fa-cart-plus"></i> <span>Nota Tagihan</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
            <?php if($rajal>0){?>
            <li class=""><a href="<?php echo base_url().'nota_tagihan.php/nota_tagihan' ?>"><i class="fa fa-circle-o"></i> Rawat Jalan</a></li>
            <?php 
            }
            if($ranap>0){
            ?>
            <li><a href="<?php echo base_url().'nota_tagihan.php/nota_tagihan/ranap' ?>"><i class="fa fa-circle-o"></i> Rawat Inap</a></li>
            <?php } 
            if($penunjang>0){
            ?>
            <li><a href="<?php echo base_url().'nota_tagihan.php/nota_tagihan/penunjang' ?>"><i class="fa fa-circle-o"></i> Penunjang</a></li>
            <?php }
            if($igd>0){
              ?>
              <li><a href="<?php echo base_url().'nota_tagihan.php/nota_tagihan/igd' ?>"><i class="fa fa-circle-o"></i> Gawat Darurat</a></li>
              <?php } ?>
          </ul>
        </li>
        <?php } ?>

        <?php if($penunjang>0) { ?>
        <li class="<?php echo $nav_3; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/permintaan_penunjang' ?>">
                <i class="fa fa-list"></i> <span>List Permintaan Penunjang</span></a>
        </li>
        <?php } ?>
        

        <?php if($rajal>0) { ?>
        <li class="<?php echo $nav_4; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/rujuk_internal' ?>">
                <i class="fa fa-list"></i> <span>List Rujuk Internal</span></a>
        </li>
        <?php } ?>

        <?php if($ranap>0) { ?>
        <li class="<?php echo $nav_5; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/pindah_kamar' ?>">
                <i class="fa fa-list"></i> <span>List Pindah Kamar</span></a>
        </li>
        <?php } ?>

        <?php if($nota_tagihan>0) { ?>        
        <!--li class="<?php //echo $nav_5; ?>">
            <a href="<?php //echo base_url().'nota_tagihan.php/list_nota_tagihan' ?>">
                <i class="fa fa-list"></i> <span>List Nota Tagihan</span></a>
        </li-->   

        <li class="<?php echo $nav_6; ?> treeview">
          <a href="#">
            <i class="fa fa-cart-plus"></i> <span>List Nota Tagihan</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
            <?php if($rajal>0){?>
            <li class=""><a href="<?php echo base_url().'nota_tagihan.php/list_nota_tagihan' ?>"><i class="fa fa-circle-o"></i> Rawat Jalan</a></li>
            <?php 
            }
            if($ranap>0){
            ?>
            <li><a href="<?php echo base_url().'nota_tagihan.php/list_nota_tagihan/ranap' ?>"><i class="fa fa-circle-o"></i> Rawat Inap</a></li>
            <?php 
            }
            if($penunjang>0){
            ?>
            <li><a href="<?php echo base_url().'nota_tagihan.php/list_nota_tagihan/penunjang' ?>"><i class="fa fa-circle-o"></i> Penunjang</a></li>
            <?php } 
            if($igd>0){
              ?>
              <li><a href="<?php echo base_url().'nota_tagihan.php/list_nota_tagihan/igd' ?>"><i class="fa fa-circle-o"></i> Gawat Darurat</a></li>
              <?php }
            ?>
            
          </ul>
        </li>
        <?php } ?>     

        <?php if($ranap>0) { ?> 
        <li class="<?php echo $nav_7; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/pasien_pulang' ?>">
                <i class="fa fa-list"></i> <span>Pasien Pulang</span></a>
        </li>       

        <li class="<?php echo $nav_8; ?>">
            <a href="<?php echo base_url().'nota_tagihan.php/list_kunjungan_unit' ?>">
                <i class="fa fa-print"></i> <span>List Pasien Keluar</span></a>
        </li> 
        <?php } ?>
    </ul>
</section>