...
class smf extends CI_Controller
{
    ...
    function index()
    {
        ...
        if (isAdminLogin()) {
            ...
            $y['contentTitle'] = "Staf Medis Fungsional (SMF)";
            $x['content'] = $this->load->view('smf/template_table', $y, true);
    ...
    function getView()
    {
        ...
        if (isset($_POST['sLike'])) {
            ...
            $condition .= "WHERE nama_smf LIKE '%$sLike%'";
        ...
        $SQL = "SELECT * FROM tbl_smf $condition ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target'] = 'tbody#getdata';
        $config['uri_segment'] = $this->uri_segment;
        $config['base_url'] = base_url() . 'smf/getView';
    ...
    function simpan()
    {
        if (isAdminLogin()) {
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                if (
                    isset($_POST['idx']) &&
                    isset($_POST['nama_smf'])
                ) {
                    $params['idx'] = trim($this->input->post('idx', true));
                    $params['nama_smf'] = trim($this->input->post('nama_smf', true));

                    if ($params['idx'] == "") {
                        $this->db->insert('tbl_smf', $params);
                        ...
                        $cekCommand = $this->db->update('tbl_smf', $params);
                        if ($cekCommand) {
                            $response['code'] = 201;
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                if (isset($_POST['idx'])) {
                    if ($_POST['idx'] == "") {
                        ...
                        $cekCommand = $this->db->query("DELETE FROM tbl_smf WHERE idx = '$_POST[idx]'");
                        if ($cekCommand) {
                            $response['code'] = 200;
