...
class kab_kota extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){
            ...
            $y['contentTitle'] = "Kabupaten / Kota";
            $y['datProvinsi'] = $this->db->get('tbl01_provinsi'); 
            $x['content'] = $this->load->view('kab_kota/template_table',$y,true);
    ...
    function getView(){
        ...
        if(isset($_POST['sLike'])){
            ...
            $condition .= "WHERE nama_kab_kota LIKE '%$sLike%'";
        ...
        $SQL = "SELECT * FROM tbl01_kab_kota $condition ORDER BY idx";
        $SQL_Count = $this->db->query("$SQL");
        $totalRec = $SQL_Count->num_rows();
        $config['target']      = 'tbody#getdata';
        $config['uri_segment']  = $this->uri_segment;
        $config['base_url']    = base_url().'mr_registrasi.php/kab_kota/getView';
        ...
        $this->load->view('kab_kota/getdata',$x);
    }
        ...
        if($ses_state){
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(
                    ...
                    if($params['idx'] == ""){
                        $cekCommand = $this->db->insert('tbl01_kab_kota',$params); 
                        if($cekCommand){
                            $response['code'] = 200;
                        ...
                        $cekCommand = $this->db->update('tbl01_kab_kota',$params); 
                        if($cekCommand){
                            $response['code'] = 201;
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(isset($_POST['idx'])){
                    if($_POST['idx'] == ""){
                        ...
                        $cekCommand = $this->db->query("DELETE FROM tbl01_kab_kota WHERE idx = '$_POST[idx]'"); 
                        if($cekCommand){
                            $response['code'] = 200;
