...
class Atm extends CI_Controller{
    ...
    function index(){
        ...
        if($ses_state){  
            ...
            $y['contentTitle'] = "Checkin Pasien Daftar Mandiri";
            $x['content'] = $this->load->view('registrasi/v_atm',$y,true);
    ...
    function data(){
        ...
        if($ses_state){  
            ...
            $data=array(
                ...
                'message'=>'OK',
                'start'=>$start,
                'row_count'=>$this->pendaftaran_model->countAtm($sekarang,$poly,$q),
                'limit'=>$limit,
                'data'=>$this->pendaftaran_model->getAtm($sekarang,$poly,$q,$start,$limit)
            );
        }else{
            $data=array(
                'status'=>false,
    ...
    function pendaftaran($id){
        $ses_state = $this->users_model->cek_session_id();
        if($ses_state){        
            $data=$this->pendaftaran_model->getPendaftaranAtm($id);
            if($data){
                $pendaftaran=array(
                    'id_pendaftaranonline'=>$data->idx,
                    'nomr'=>$data->nomr,
                    'no_ktp'=>$data->no_ktp,
                    ...
                    'nama_kab_kota'=>$data->nama_kab_kota,
                    'nama_kecamatan'=>$data->nama_kecamatan,
                    'nama_kelurahan'=>$data->nama_kelurahan,
                    'jns_layanan'=>$data->jns_layanan,
                    'tgl_masuk'=>$data->tgl_masuk,
                    'tgl_daftar'=>$data->tgl_daftar,
                    'id_ruang'=>$data->id_ruang,
                    'nama_ruang'=>$data->nama_ruang,
                    'id_rujuk'=>$data->id_rujuk,
                    'rujukan'=>$data->rujukan,
                    'no_rujuk'=>$data->no_rujuk,
                    'id_cara_bayar'=>$data->id_cara_bayar,
                    'cara_bayar'=>$data->cara_bayar,
                    'id_jenis_peserta'=>$data->id_jenis_peserta,
                    'jenis_peserta'=>$data->jenis_peserta,
                    'no_bpjs'=>$data->no_bpjs,
                    'pjPasienNama'=>$data->pjPasienNama,
                    'pjPasienUmur'=>$data->pjPasienUmur,
                    'pjPasienPekerjaan'=>$data->pjPasienPekerjaan,
                    'pjPasienAlamat'=>$data->pjPasienAlamat,
                    'pjPasienTelp'=>$data->pjPasienTelp,
                    'pjPasienHubKel'=>$data->pjPasienHubKel,
                    'pjPasienDikirimOleh'=>$data->pjPasienDikirimOleh,
                    'pjPasienAlmtPengirim'=>$data->pjPasienAlmtPengirim,
                    'dokterJaga'=>$data->dokterJaga,
                    'namaDokterJaga'=>$data->namaDokterJaga,
                    'user_daftar'=>$this->session->userdata('get_uid'),
                    'status_pasien'=>1,
                    'session_id' => getSessionID()
                );
                $this->db->insert('tbl02_pendaftaran',$pendaftaran);
                $idx=$this->db->insert_id();
                $this->db->where('idx',$idx);
                $res=$this->db->get('tbl02_pendaftaran')->row();
                if($res){
                    $antrian=array(
                        'id_daftar'=>$res->id_daftar,
                        'reg_unit'=>$res->reg_unit
                    );
                    $this->db->where('id_pendaftaranonline',$data->idx);
                    $this->db->update('tbl02_antrianv2',$antrian);
                    header('location:'.base_url() ."mr_registrasi.php/registrasi/reg_success?uid=".encrypt_decrypt('encrypt',$res->reg_unit,true));
                }else{
                    header('location:'.base_url() ."mr_registrasi.php/registrasi/atm");
                }
            }else{
