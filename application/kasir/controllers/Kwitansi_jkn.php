...
class kwitansi_jkn extends CI_Controller {
    ...
    public function index(){
        ...
        if($ses_state){
            ...
            $y['contentTitle'] = "Kwitansi Bukan Umum";        
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if(
                    ...
                    $params = array(
                        ...
                        'no_bpjs'       => trim($this->input->post('no_bpjs',true)),
                        'no_sep'        => trim($this->input->post('no_sep',true)),
            ...
            if(isset($_GET['kode'])){
                ...
                if ($kode=="") {
                    ...
                    if($cek->num_rows() > 0){
                        ...
                        $this->load->view('kwitansi_jkn/invoice_v2',$x);
                    }else{
                        echo "<script>alert('Ops. No. Invoice $kode tidak ditemukan');
                        window.close();</script>";
                    }
            ...
            if(isset($_GET['kode'])){
                ...
                if ($kode=="") {
                    ...
                    if($cek->num_rows() > 0){
                        ...
                        $this->load->view('kwitansi_jkn/invoice_jkn',$x);
                    }else{
                        echo "<script>alert('Ops. No. Invoice $kode tidak ditemukan');
                        window.close();</script>";
                    }
            ...
            if(isset($_GET['kode'])){
                ...
                if ($kode=="") {
                    ...
                    if($cek->num_rows() > 0){
                        ...
                        $this->db->where('kode_item',$kode);
                        $x['datDetail'] = $this->db->get('tbl05_kwitansi_detail_item');
                        $this->load->view('kwitansi_jkn/print_layanan',$x);
                    }else{
                        echo "<script>alert('Ops. No. Transaksi $kode tidak ditemukan');
                        window.close();</script>";
                    }*/
                    $this->db->where('reg_unit', $kode);
                    $x['datMaster']=$this->db->get('tbl02_pendaftaran');
                    $x['kode_item']=$kode;
                    $this->db->where('kode_item',$kode);
                    $x['datDetail'] = $this->db->get('tbl05_kwitansi_detail_item');
                    $this->load->view('kwitansi_jkn/print_layanan',$x);
            ...
            if(isset($_GET['kode'])){
                ...
                if ($kode=="") {
                    ...
                    if($cek->num_rows() > 0){
                        ...
                        $this->db->where('kode_item',$kode);
                        $x['datDetail'] = $this->db->get('tbl05_kwitansi_detail_item');
                        $this->load->view('kwitansi_jkn/print_farmasi',$x);
                    }else{
                        echo "<script>alert('Ops. No. Transaksi $kode tidak ditemukan');
                        window.close();</script>";
                    }
